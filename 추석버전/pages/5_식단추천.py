...
def get_prompt_template(query):
    ...
    if "식단" in query:
        ...
        # 프롬프트 생성
        prompt = Template(f"""
        사용자가 원하는 기간이 있으면 그 기간, 없으면 하루의 식단을 추천해 줘.
        다음은 예시 식단이야:
        아침: {breakfast}
        점심: {lunch}
        저녁: {dinner}
        
        {pdf_content2}의 내용을 참고해서 고혈압 확률이 20%가 넘는 경우 나트륨 1000mg 이하의 식단을 
        {docx_file_path}에서만 찾아서 제안해줘. 
        고혈압 확률이 20% 미만이라면 나트륨 제한 없이 자유로운 식단을 {docx_file_path}에서만 추천해줘.
        """)
    # "레시피"가 포함된 경우
    elif "레시피" in query:
        prompt = Template(f"""
